...
CUDA_VISIBLE_DEVICES=1 accelerate  launch --config_file ./accelerate_config/config_$image_name.yaml  --main_process_port 29531 pretrain_inversion_appearance.py \
  ...
  --clip_model_name_or_path="ViT-B-32::/data1/huggingface_ckpts/laion/CLIP-ViT-B-32-laion2B-s34B-b79K/open_clip_pytorch_model.bin" \
  --dataset_class="dog" \
  ...
  --output_dir="../OSTAF_Output/SD1.4/1/appearance-inversion-$image_name-$domain_embed_scale-$batch_size" \
  --placeholder_token="*s" \
  --placeholder_token_shape="*m" \
  --placeholder_token_appearance="*a" \
  --pretrained_model_name_or_path="/data1/huggingface_ckpts/CompVis/stable-diffusion-v1-4" \
  --prompt_template="joint" \
  --reg_lambda=0.01 \
  --resolution=512 \
  --student_prompt="a $domain_class_token in the style of *a" \
  --train_batch_size=$batch_size \
  --origin_img_path="/data1/ye_project/SigStyle/data/style/$image_name" \
