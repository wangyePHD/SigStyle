...
class ShapeInvEncoder(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(
            self,
    ...
    def forward(self, x, unet_down_block_samples: tuple):
        ...
        x = self.preprocess(x)
        cls_output, hidden_states = self.clip_vision(x)

        hidden_states = hidden_states[:, 1::2, :] # take every 2nd layer 
        hidden_states = torch.cat([cls_output.unsqueeze(1), hidden_states], dim=1)
...
class AppInvEncoder(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(
            self,
            word_embedding_dim=768,
            arch="ViT-B-32", 
            version="/home/wangye/huggingface_ckpts/CLIP-ViT-B-32-laion2B-s34B-b79K/open_clip_pytorch_model.bin",
            antialias=False,
            freeze_clip_vision=True,
    ):
    ...
    def forward(self, x):
        ...
        clip_hidden_states = self.act(cls_output)
        clip_hidden_states = self.final_linear(clip_hidden_states)
        # final linear layer
...
class MaskEncoder(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(
            self,
            word_embedding_dim=768,
...
if __name__ == '__main__':
    ...
    import sys
    sys.path.append("../../ShapeInversion")
    ...
    model_id = "/home/wangye/huggingface_ckpts/stable-diffusion-v1-4"
