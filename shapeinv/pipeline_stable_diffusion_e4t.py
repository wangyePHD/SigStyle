...
class StableDiffusionE4TPipeline_shape_mask(StableDiffusionPipeline):
    def __init__(
            self,
            vae,
            text_encoder,
        ...
        if not already_added_placeholder_token:
            ...
            text_encoder.resize_token_embeddings(len(tokenizer))
        self.placeholder_token = e4t_config.placeholder_token
        self.placeholder_token_shape = e4t_config.placeholder_token_shape
        
        self.placeholder_token_id = tokenizer.convert_tokens_to_ids(e4t_config.placeholder_token)
        self.placeholder_token_shape_id = tokenizer.convert_tokens_to_ids(e4t_config.placeholder_token_shape)
        
        # save class embed
        domain_class_token_id = self.tokenizer(e4t_config.domain_class_token, add_special_tokens=False, return_tensors="pt").input_ids[0]
        shape_token_id = self.tokenizer(e4t_config.shape_class_token, add_special_tokens=False, return_tensors="pt").input_ids[0]
        
        # get class token embedding
        self.class_embed = text_encoder.get_input_embeddings()(domain_class_token_id.to(text_encoder.device))
        self.shape_embed = text_encoder.get_input_embeddings()(shape_token_id.to(text_encoder.device))
        self.domain_embed_scale = e4t_config.domain_embed_scale
    ...
    def prepare_for_e4t(self, prompt, device):
        ...
        try:
            placeholder_token_id_idx = input_ids[0].tolist().index(self.placeholder_token_shape_id)
        ...
        inputs_embeds = self.text_encoder.get_input_embeddings()(input_ids.to(device)).to(dtype=self.text_encoder.dtype, device=device)
        return dict(
            placeholder_shape_token_id_idx=placeholder_token_id_idx,
            encoder_hidden_states_for_e4t=encoder_hidden_states_for_e4t,
            inputs_embeds=inputs_embeds
        )
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # student model
                shape_embed_filter = shape_filter(domain_embed)
                student_shape_embed = self.shape_embed.clone().expand(bsz, -1).to(device) + domain_embed_scale * shape_embed_filter
                
                inputs_embeds_forward = e4t_inputs["inputs_embeds"].expand(bsz, -1, -1).clone().to(dtype=self.text_encoder.dtype, device=device)
                inputs_embeds_forward[:, e4t_inputs["placeholder_shape_token_id_idx"], :] = student_shape_embed
...
class StableDiffusionE4TPipeline_appearance(StableDiffusionPipeline):
    def __init__(
            self,
            vae,
            text_encoder,
        ...
        if not already_added_placeholder_token:
            ...
            text_encoder.resize_token_embeddings(len(tokenizer))
        self.placeholder_token = e4t_config.placeholder_token
        self.placeholder_token_appearance = e4t_config.placeholder_token_appearance
        
        self.placeholder_token_id = tokenizer.convert_tokens_to_ids(e4t_config.placeholder_token)
        self.placeholder_token_appearance_id = tokenizer.convert_tokens_to_ids(e4t_config.placeholder_token_appearance)
        
        # save class embed
        domain_class_token_id = self.tokenizer(e4t_config.domain_class_token, add_special_tokens=False, return_tensors="pt").input_ids[0]
        appearance_token_id = self.tokenizer(e4t_config.appearance_class_token, add_special_tokens=False, return_tensors="pt").input_ids[0]
        
        # get class token embedding
        self.class_embed = text_encoder.get_input_embeddings()(domain_class_token_id.to(text_encoder.device))
        self.appearance_embed = text_encoder.get_input_embeddings()(appearance_token_id.to(text_encoder.device))
        self.domain_embed_scale = e4t_config.domain_embed_scale
    ...
    def prepare_for_e4t(self, prompt, device):
        ...
        try:
            placeholder_token_id_idx = input_ids[0].tolist().index(self.placeholder_token_appearance_id)
        ...
        inputs_embeds = self.text_encoder.get_input_embeddings()(input_ids.to(device)).to(dtype=self.text_encoder.dtype, device=device)
        return dict(
            placeholder_appearance_token_id_idx=placeholder_token_id_idx,
            encoder_hidden_states_for_e4t=encoder_hidden_states_for_e4t,
            inputs_embeds=inputs_embeds
        )
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                student_appearance_embed = self.appearance_embed.clone().expand(bsz, -1).to(device) + domain_embed_scale * appearance_embed_filter
                
                inputs_embeds_forward = e4t_inputs["inputs_embeds"].expand(bsz, -1, -1).clone().to(dtype=self.text_encoder.dtype, device=device)
                inputs_embeds_forward[:, e4t_inputs["placeholder_appearance_token_id_idx"], :] = student_appearance_embed
                ...
                noise_pred = student_unet(
                    ...
                    encoder_hidden_states=prompt_embeds,
                    cross_attention_kwargs=cross_attention_kwargs,
                    appearance_feature_dict=None
                ).sample
                
                
                
                
                # noise_pred_bkg = ddim_pipe.unet(latents, t, encoder_hidden_states=context_ddim).sample
                
                # noise_pred_bkg = student_unet(latents, t, encoder_hidden_states=context_ddim).sample
                ...
                # mask_tensor_inter = F.interpolate(mask_tensor,size=(64,64), mode="nearest")
                # mask_tensor_inter = mask_tensor_inter.repeat(1,noise_pred.shape[1],1,1)
                # mask_tensor_inter = 1 - mask_tensor_inter
                
                
                # for name, attn_proc in student_unet.attn_processors.items():
                    
                #     if "attn2" in name:
                #         attn_map = attn_proc.attn_map
                #         import ipdb; ipdb.set_trace()
                ...
                if do_classifier_free_guidance:
                    ...
                    noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond)
                    # bkg infer
                    # noise_pred = noise_pred * mask_tensor_inter + (1-mask_tensor_inter) * noise_pred_bkg
                ...
                if i == len(timesteps) - 1 or ((i + 1) > num_warmup_steps and (i + 1) % self.scheduler.order == 0):
                    progress_bar.update()
                    if callback is not None and i % callback_steps == 0:
                        print(f"Calling callback at step {i} with t={t} and latents")
                        callback(i, t, latents)
