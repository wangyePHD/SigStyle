...
from shapeinv.weightoffsets import WeightOffsets
...
class CrossAttention(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, appearance_feature_dict=None, **cross_attention_kwargs):
        ...
        return self.processor(
            ...
            ##############################
            appearance_feature_dict=appearance_feature_dict,
            ##############################
...
class CrossAttnProcessor:
    def __call__(
        ...
        query = F.linear(hidden_states, attn.to_q.weight * (1 + weight_wo * attn.wo_q()), bias=attn.to_q.bias)
        ######################
        ...
        key = F.linear(encoder_hidden_states, attn.to_k.weight * (1 + weight_wo * attn.wo_k()), bias=attn.to_k.bias)
        # value = attn.to_v(encoder_hidden_states)
        value = F.linear(encoder_hidden_states, attn.to_v.weight * (1 + weight_wo * attn.wo_v()), bias=attn.to_v.bias)
        ######################
        ...
        if not flag_cross_attn:
            setattr(attn, "self_hidden_states", hidden_states)
    ...
    def __call__(self, attn: CrossAttention, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        if flag_cross_attn:
            ...
            if attention_mask is not None:
                attn_map = attn_map + attention_mask
            attn_map = attn_map.softmax(-1)
            # attn_map = F.dropout(attn_map, p=0.2)
            setattr(attn, "xattn", attn_map)
